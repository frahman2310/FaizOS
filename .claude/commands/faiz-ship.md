...
5. **Auto-close the loop immediately — do NOT wait for him to ask:**
   ...
   - Post the **comprehensive** Revision Note (the full format in `/faiz-build`: why-it-matters, what-you-built + core mechanism/code, the full concept chain *with worked examples*, key formulas, gotchas, payoff, curriculum-placement + next — complete, never condensed), save it (`faizos_save_revision`), and **directly under it show the course progress bar** from `faizos_progress` (print its `rendered` block).
