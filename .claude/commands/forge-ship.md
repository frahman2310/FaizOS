...
2. Ship it appropriately: commit, and if it's meant to be public, push. **Any push/publish is outward-facing** — only push to an **already-configured remote** (check `forge_config`). If none is set, commit locally and tell him how to add one (or run `gh repo create`, which needs his go-ahead).
3. Call `forge_ship` with the mission id (or none = current) and `ship_url` if there is one.
...
5. Tell him to run `/forge-analyze` to bank the skills, and tease the suggested next.
