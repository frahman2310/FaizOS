...
| C30 | 09-14 | "Each lesson can still have a build (just needs to be one I understand and build through my concepts and decisionmaking rather than my code writing)" | superseded 09-24 by INTEGRATED.md: builds are the Saturday whole task and the design track's Choices step |
| C31 | 09-15 | "decision 3 and 4 depend on where the majority of my users are which is unknown so correct that for the future and also explain these questions more throughly" | kept in v2: every design and production given states its source or unknown (check_unit: runs, facts, given:) |
| C32 | 09-15 | "I liked the build part, i want it to be more throughly explained (the implications of my decisions and the choices should be explained much better)"; "parse this lesson, add to the feedback loop and improve the next lessons (make this automatic)" | kept in v2: design Choices step states each option's effect on the target number |
...
| C35 | 09-18 | "Option C obviously (decisions cannot be this obvious)" on L5 build decisions 1 and 2 | kept in v2: design and production options must each win on some dimension (structures/system-design.md) |
...
| C39 | 09-22 | "I don't mind predicting if I'm given all the data I need to make my prediction there. The decisions need to be harder, more like actual engineering decisions and more realistic" | kept in v2: predictions come with all the data needed (engine predict before scored steps) |
...
| C41 | 09-24 | On case-01: "What am I supposed to do here"; then "You need to redefine the whole problem. Rephrase the original prompt including the clear job description (commit this)" | superseded 09-24: investigation mode retired; every unit states its goal in its first step |
| C42 | 09-24 | After 4 open moves in case-01: "I don't understand this exercise we're doing. seems unstructured and pointless" | superseded 09-24: investigation mode retired; fixed per-skill structures replace it |
...
| C46 | 09-24 | "5 hours a week is nothing, workload is not an issue" | standing: dose is set by what adds learning (spacing, diminishing returns), not by time |
| C47 | 09-25 | "Don't do that days dont matter" (on a weekday timetable leaving Friday empty) | applied at once (pacing): engine.py runs sittings in the INTEGRATED week order whenever he sits down; weeks count sittings; the 7-day cold check stays calendar-based (spacing is the evidence) |
| C48 | 09-25 | "explain the code and why it's structured like that in every exercise" | applied at once (delivery): every code block in a unit step comes with a **How this code works** block saying what each part does and why it is built that way, without giving the answer; enforced by learn/check_unit.py |
| C49 | 09-25 | "We made a whole comprehensive plan, why aren't you sticking to it" / "you didn't show me a worked example, you're overloading me immediately, you didn't explain the code ... go back and research how code is taught by experts ... audit this entire curriculum and all the rules ... bring it up to that standard" | applied at once, overrides the 8-week freeze (his explicit instruction): teaching paused; code structure re-researched from expert practice and rebuilt; whole curriculum, rules, questions and testing audited against expert practice; first code session (code-01) voided, not scored |
